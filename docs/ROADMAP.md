...
- [ ] Promote Showly local History/Watchlist mutations to first-class three-way mutations instead of relying on Trakt as an intermediate. Schema-44 durable per-provider ACKs, direct Showly -> Floppy fast path, shared execution gate, and pre-snapshot outbox drain are verified through Fork CI #50. The final local-History patch additionally removes My Shows/collection scheduling gates, resolves parent TV TMDB identity correctly, and seeds legacy unexported episodes during 43 -> 44 migration; CI/device validation is pending.
