...
- [ ] Promote Showly local History/Watchlist mutations to first-class three-way mutations instead of relying on Trakt as an intermediate. Schema-44 durable per-provider ACKs, direct Showly -> Floppy fast path, shared execution gate, and pre-snapshot outbox drain are verified through Fork CI #50. The final local-History patch also removes My Shows/collection scheduling gates, snapshots parent TV TMDB + season/episode identity into the outbox so unfollowed removals survive local row deletion, and seeds legacy unexported episodes during 43 -> 44 migration; final CI/device validation is pending.
- [ ] Reduce full-sync latency. The duplicate History/Watchlist post-pass and Floppy History N+1 detail scan are verified through Fork CI #50; measure on-device against the prior ~65.3 s run and then optimize Ratings/Custom Lists only if needed.
