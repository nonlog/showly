...
1. Verify the UI scheduling-gate removal, parent-show TMDB fix, all-watched-episodes exporter, and schema-44 recovery seed in Fork CI; CI #50 is the latest fully green baseline and the next run supersedes #51.
2. Install only the consolidated final Debug APK. Confirm Room schema 44, confirm the migration-created episode outbox rows drain to both Trakt and Floppy, and verify the reported local S1E2 is present remotely without a second full sync.
3. Use a disposable unfollowed-show episode plus a disposable movie Watchlist item to reproduce the exact local UI-origin path; both should reach Trakt and Floppy automatically while legacy `trakt_quick_sync_enabled=0`; measure latency.
4. Re-run one manual full bridge sync and compare wall time against the prior ~65.3 s baseline; only then optimize Ratings/Custom Lists further if needed.
