...
1. Verify the schema-44 local two-provider outbox, Hilt wiring, flat-history parser, and reduced full-sync pipeline in Fork CI.
2. Install the resulting Debug APK and reproduce the exact reported path with disposable data: add one History event and one Watchlist item in Showly, then verify both appear in Trakt and Floppy without running a full manual sync; measure end-to-end latency.
3. Measure full manual-sync wall time after removing the duplicate History/Watchlist post-pass and Floppy History N+1 requests, then profile Ratings/Custom Lists if the remaining duration is still excessive.
4. Resume the controlled Ratings and History/rewatch deletion/re-add matrices after the local-origin regression is closed.
