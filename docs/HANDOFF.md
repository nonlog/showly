...
1. Verify the parent-show TMDB fix and all-watched-episodes export fix in Fork CI, then install only the consolidated final Debug APK.
2. Confirm Room schema 44 on-device and reproduce the exact reported local-origin path with disposable data: local Watchlist + local watched episode should reach Trakt and Floppy automatically without a full manual sync; measure latency.
3. Re-run one manual full bridge sync and compare wall time against the prior ~65.3 s baseline. If still excessive, profile Ratings/Custom Lists next; on the current Floppy account History was the dominant HTTP fan-out.
4. Confirm the pre-existing House of the Dragon S1E2 local history is recovered to both remotes, then resume Ratings/History deletion/re-add matrices.
